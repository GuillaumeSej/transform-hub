...
/**
 * Sélecteur de plage de dates ISO (YYYY-MM-DD) — inputs `type="date"` natifs pour rester léger
 * et accessible (pas de dépendance calendrier tierce). Utilisé par le Dashboard RH pour cadrer
 * la période analysée (Gooduelle "Du → Au"), mais volontairement générique pour être réutilisable
 * ailleurs (Finance, Actions...).
 *
 * L'état vit dans l'appelant — ce composant ne persiste rien. Pour un état partageable dans
 * l'URL, l'appelant utilise `useSearchParams`/`useRouter` et passe ses handlers.
 */

import { useMemo } from "react";

/** Compte les mois, trimestres et exercices "fiscaux calendaires" (jan-déc) couverts par une
 *  plage — sert au libellé indicatif "N mois · N trimestres · N exercices" de la barre. */
...
export function DateRangePicker({
  ...
  ariaLabelTo?: string;
  /** Afficher le résumé "N mois · N trimestres · N exercices" sous les inputs. */
  showSummary?: boolean;
}) {
