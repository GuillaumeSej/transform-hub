...
describe("mockActionMigration", () => {
  ...
  it("preserves each parent lever net savings after consolidating migrated actions", () => {
    // netSavings = savings − OPEX récurrent (voir lib/leverConsolidate.ts) : CAPEX et OPEX
    // one-off ne rentrent jamais dans le net annualisé.
  ...
  it("reconciles every migrated lever's impacts to the cent with its own financial fields", () => {
    ...
    // impacts d'actions (correctement signés/typés, convention netSavings = savings − OPEX récurrent de
